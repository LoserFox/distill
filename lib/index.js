...
export class DistillLedger {
    ...
    async record(sessionId, record) {
        await mkdir(this.root, { recursive: true });
        await appendFile(join(this.root, `${sessionId}.jsonl`), `${JSON.stringify(record)}\n`, { encoding: 'utf8' });
    }
    /**
 ...
 */
async function reviewOnce(ctx, config, ledger, agent, session) {
